  ...
  return (
    <>
      <div className="w-full container mx-auto px-4 grid grid-cols-12 my-2 sm:my-4">
        <div
          className="col-span-full lg:col-span-8 w-full aspect-[2/1] object-fill relative rounded-md h-full sm:overflow-clip"
              ...
              <div className="bg-secondary sm:bg-transparent bg-gradient-to-t sm:from-black h-full full sm:absolute z-40 justify-start flex flex-col items-start sm:justify-end w-full gap-2 px-2 py-4 sm:p-4 bottom-0 rounded-b-md sm:rounded-b-2xl">
                <div className="grid grid-cols-6 rounded-md sm:rounded-2xl">
                  ...
                  <div className="col-span-4 sm:col-span-5 overflow-hidden h-full pl-2 w-full sm:w-full flex flex-col items-start sm:justify-end space-y-2 relative">
                    ...
                    <div
                      className="px-2 sm:p-2 border border-highlight flex items-center w-fit justify-center text-highlight z-50 rounded-sm"
                      onClick={(e) => {
              ...
              <div className="relative w-full">
                <div className="absolute w-full z-40 bottom-0 sm:top-0 h-fit">
                  <div className="flex w-full justify-between items-end">
                    <button
                      className="p-4 rounded-tr-md sm:rounded-br-md sm:rounded-tr-none sm:p-4 bg-stroke/50 cursor-pointer"
                      onClick={(e) => {
                        e.stopPropagation();
                        num >= 1 && setNum(num - 1);
                      }}
                    ...
                    <button
                      className="p-4 rounded-tl-md sm:rounded-bl-md sm:rounded-tl-none sm:p-4 bg-stroke/50 cursor-pointer"
                      onClick={(e) => {
                        e.stopPropagation();
                        num !== 9 && setNum(num + 1);
                      }}
                ...
                <img
                  ...
                  alt=""
                  className="object-cover w-full aspect-[2/1] rounded-t-md"
                />
              </div>
            </div>
          ) : (
            <div className="w-full aspect-[2/1] bg-gray-300 animate-pulse rounded-md"></div>
          )}
        </div>
        <div className="col-span-4 aspect-square pl-4 overflow-clip rounded-md w-full hidden lg:block space-y-2">
          <div className="h-full overflow-y-scroll space-y-2 rounded-md">
            <div className="font-bold text-2xl p-4 z-10 bg-secondary text-highlight rounded-t-md rounded-b-md sticky top-0">
              <div>Now Playing</div>
            </div>
            {movieData.length !== 0 ? (
                  ...
                  >
                    ...
                    {unit.poster_path ? (
                      <img
                        ...
                        alt=""
                        className="w-1/5 object-cover rounded-sm"
                      />
                    ) : (
                      <div className="aspect-[3/4] w-full bg-gray-300 font-bold text-xl">
                        Image not Found
                      </div>
                    )}
                    <div>
                      ...
                      <div className="text-highlight flex gap-2">
                        {execute
                          .handleFilter(unit.genre_ids, genres)
                          .join(", ")}
                      </div>
                    </div>
                  </div>
              ...
              <SkeletonLandscape behavior="full" />
            )}
          </div>
        </div>
      </div>
